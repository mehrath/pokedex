...
public class PokemonStorage {
    ...
    public String getPokeURL(String pokeName){
        return "https://pokeapi.co/api/v2/pokemon/" + pokeName +"/";
    }
    public Map getPokeSprites(String pokeName) throws IOException {
        URLConnection connection = new URL(getPokeURL(pokeName)).openConnection();
        InputStream is = connection.getInputStream();
        System.out.println(connection.getContentType());
        byte[] response = is.readAllBytes();
        String resp = new String(response);
    ...
    public Integer getPokeId(String pokeName) throws IOException {
        URLConnection connection = new URL(getPokeURL(pokeName)).openConnection();
        InputStream is = connection.getInputStream();
        System.out.println(connection.getContentType());
        byte[] response = is.readAllBytes();
